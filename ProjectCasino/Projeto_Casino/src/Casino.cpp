...
//Destruição do casino e a lista das pessoas e das maquinas
...
/*list<Jogador *> *Casino::Jogadores_Mais_Frequentes (){ //Função que vai definir o jogador que teve mais interações com as máquinas do casino no dado dia
...
//Mostra a informação sobre o casino juntamente com a informação dos jogadores e maquinas
void Casino::show(ostream &f)
    {
        ...
        cout << "Quantidade de Máquinas avariadas: " << List_Maquina_avariada.size() << endl;
        ...
        cout << "Detalhes das Máquinas:" << endl;
        for (auto maquina : List_Maquina)
        {
            // Imprima informações específicas da máquina, se necessário
        }
...
//Função para mostrar quais as maquinas que mais trabalharam
...
//Função para ver qual dos jogadores mais ganhou
...
void Casino::VerificarEstadoMaquina(const string& nomeMaquina) {
    // Procurar na lista de máquinas avariadas
    for (const auto& maquina : List_Maquina_avariada) {
        if (maquina->GetID() == nomeMaquina) {
            cout << "A maquina " << nomeMaquina << " está estragada e não pode ser utilizada." << endl;
            return;
        }
    }

    // Procurar na lista de máquinas disponíveis
    for (const auto& maquina : List_Maquina) {
        if (maquina->GetID() == nomeMaquina) {
            cout << "A maquina " << nomeMaquina << " está disponível." << endl;
            return;
        }
    }

    // Se não encontrou a máquina em nenhuma lista
    cout << "Não foi encontrada uma maquina com o nome " << nomeMaquina << "." << endl;
}

//recebe a informação retirada do ficheiro xml e cria uma maquina nova com essa informação
Maquina* Casino::criarMaquina(string nomeMaquina, int probabilidadeGanhar, int premio, int x, int y, int tempAviso) {
    // Aqui, com base no nome da máquina, você decide qual tipo de máquina criar
...
//função principal onde tudo é chamado
void Casino::MenuPrincipal() {

    setlocale(LC_ALL, "Portuguese_Portugal.1252");
    srand(time(0));
    Casino meuCasino(Nome_Casino); //lembrar que o a ciração assim automaticamente passa no destruir
    ...
    int numeroAleatorio = std::rand() % 100 + 1;
        ...
        do {
            ...
            std::cout << "|=======================================|\n";
            std::cout << "|                                       |\n";
            std::cout << "| 1. Abrir o casino (1 pessoa de cada)  |\n";
            std::cout << "| 2. Informações do casino              |\n";
            ...
            std::cout << "| 7. Ver se uma maquina está disponivel |\n";
            ...
            switch (opcao) {
                case 1:
                        {
                            ...
                            while (!_kbhit());  // Aguardar até que uma tecla seja pressionada
                            int tecla = _getch();  // Obter o código da tecla pressionada
                            if (tecla == 13) {  // 13 é o código ASCII para Enter
                                system("cls");  // Limpar a tela
                                // Adicione outras verificações ou condições para sair do loop, se necessário
                            }
                        }
                        break;
                case 2:
                ...
                case 5:
                       while (true) {
                        for (int i = 12; i <= 72; ++i) {  // Supondo que você quer repetir de 12 até 24 (inclusive)
                    ...
                    break;
                case 6:
                    cout << "Memória Total Usada: " << memoriaTotal << " bytes" << std::endl;
                    cout << "Pressione Enter para continuar..." << endl;
                    cin.ignore(); // Limpar o buffer do teclado
                    cin.get();
                    system("cls");
                    break;
                case 7:{
                    cout << "Digite o nome da máquina: ";
                    cin.ignore(); // Limpar o buffer do teclado
                    string nomeMaquina;
                    getline(std::cin, nomeMaquina);  // Para lidar com espaços no nome

                    // Verificar o estado da máquina
                    meuCasino.VerificarEstadoMaquina(nomeMaquina);
                    cout << "Pressione Enter para continuar..." << endl;
